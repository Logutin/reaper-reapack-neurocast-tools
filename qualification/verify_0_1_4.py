"""Read-only checks for the 0.1.4 payload, candidate index, and installed receipt."""
import argparse
...
SOURCE_COMMIT = "9d734e197bea078e112c85eebbe7dd5283b0bf39"
PREVIOUS = "d1ad9cfd89548b23ea76a85badccf2294ce50ce3"
DISPOSABLE = Path("C:/extra_Reapers/Reaper_Empty_01")
CHANGED = {
    "scr/automix_tool.lua",
    "scr/modules-neurocast/ReaperX_Import_Media_Manually.lua",
    "scr/modules-neurocast/auphonic_api_via_neurocast.lua",
    "scr/modules-neurocast/automix_tool_languages.lua",
    "scr/modules-neurocast/zip_archive.lua",
    "scr/modules-neurocast/zip_extract_writer.lua",
    "scr/modules-neurocast/Curl.lua",
    "scr/modules-neurocast/mvsep_tool_languages.lua",
    "scr/mvsep_tool.lua",
}
PAIR = {
...
def validate_payload():
    ...
    check(manifest["package"]["version"] == lock["package"]["version"] == "0.1.4", "package version")
    ...
    check(set(lock["tracked_lua"]["byte_exact_changed_files"]) == CHANGED, "nine changed/new inputs")
    contents, crlf = {}, set()
    for item in inputs:
    ...
    check(len(crlf) == 9 and crlf == set(lock["tracked_lua"]["historical_crlf_preserved_distribution_files"]), "nine historical CRLF files")
    ...
    print("PASS payload: 60 Lua syntax/content/closure; 9 exact changed/new and 51 unchanged Lua; official 7-Zip 26.03 pair; other binaries/notices unchanged; 192 rows; 68 Windows files/14 actions")
    return manifest
...
def validate_index(path, candidate, manifest):
    ...
    check(len(elements) == len(versions) == 5, "five distinct versions")
    ...
    sources = versions["0.1.4"].findall("source")
    ...
    print("PASS index: 192 pinned 0.1.4 sources; all four historical versions unchanged")
...
def validate_installed(manifest):
    ...
    with sqlite3.connect(registry.as_uri() + "?mode=ro", uri=True) as db:
        ...
        check(len(entries) == 1 and entries[0][1] == "0.1.4", "installed receipt version")
    ...
    print("PASS disposable receipt: 0.1.4; 68 byte-exact owned files; 14 exactly-once Main actions. Owner GUI acceptance remains separate.")
