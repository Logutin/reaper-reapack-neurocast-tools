...
--
-- Optional opts:
--   opts.curl_submit_fn(req, on_done, submit_opts) - defaults to modules-neurocast.Curl.curl_submit
...
if not ok_json or type(json) ~= "table" or type(json.decode) ~= "function" then
  error("neurocast_auth: JSON module not available (required 'modules-neurocast.json'): " .. tostring(json))
end

local ok_base64, base64_codec = pcall(require, "modules-neurocast.base64_encode_decode")
if not ok_base64 or type(base64_codec) ~= "table" or type(base64_codec.decode_url) ~= "function" then
  error("neurocast_auth: base64 module not available (required 'modules-neurocast.base64_encode_decode'): " .. tostring(base64_codec))
end
...
if not ok_curl or type(Curl) ~= "table" or type(Curl.curl_submit) ~= "function" then
  error("neurocast_auth: Curl module not available (required 'modules-neurocast.Curl'): " .. tostring(Curl))
end

local ok_util, Util = pcall(require, "modules-neurocast.Util")
if not ok_util or type(Util) ~= "table" then
  error("neurocast_auth: Util module not available (required 'modules-neurocast.Util'): " .. tostring(Util))
end
