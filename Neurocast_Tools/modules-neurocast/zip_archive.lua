...
local ExtractWriter = require("modules-neurocast.zip_extract_writer")
...
local function append_retained_diagnostics_warning(warning_parts, listing_path, extract_log_path, list_script_path, extract_script_path)
  ...
  if type(extract_script_path) == "string" and extract_script_path ~= "" then
    parts[#parts + 1] = "Extract command script: " .. tostring(extract_script_path)
  end
...
local function build_stage_failure(stage_label, archive_tool_path, zip_path, diagnostic_path, exec_result, extra_message)
  ...
  if type(exec_result) == "table" and type(exec_result.script_path) == "string" and exec_result.script_path ~= "" then
    parts[#parts + 1] = "Command script: " .. tostring(exec_result.script_path)
  end
...
-- Keep archive identity separate from the allocated on-disk filename. Reserve
-- planned names in this batch as well as names already present in the folder.
function ZipArchive.allocate_audio_paths(audio_files, output_dir)
  local reserved, renamed = {}, {}
  for _, item in ipairs(audio_files) do
    local original = item.filename
    local candidate = Files.bump_to_unique_path(Util.path_join(output_dir, original))
    local suffix = 0
    while reserved[string.lower(basename(candidate))] do
      suffix = suffix + 1
      local stem, ext = original:match("^(.*)(%.[^.]+)$")
      candidate = Files.bump_to_unique_path(Util.path_join(output_dir, stem .. "_" .. suffix .. ext))
    end
    reserved[string.lower(basename(candidate))] = true
    item.original_filename = original
    item.filename = basename(candidate)
    item.path, item.output_dir = candidate, output_dir
    if item.filename ~= original then
      renamed[#renamed + 1] = {original_filename=original, filename=item.filename, stem_id=item.stem_id, path=candidate}
    end
  end
  return renamed
end

local function build_windows_list_command(archive_tool_path, zip_path, listing_path)
  ...
  return quoted_7z .. " l -slt -sccUTF-8 " .. quoted_zip .. " > " .. quoted_listing .. " 2>&1"
end

local function build_posix_list_command(archive_tool_path, zip_path, listing_path)
  local quoted_unzip = Util.shell_quote(archive_tool_path)
  local quoted_zip = Util.shell_quote(zip_path)
  local quoted_listing = Util.shell_quote(listing_path)
  return quoted_unzip .. " -Z1 " .. quoted_zip .. " > " .. quoted_listing .. " 2>&1"
end
...
function ZipArchive.extract_audio_files(zip_path, out_dir)
  ...
  if Util.is_windows() then
    list_command = build_windows_list_command(archive_tool_path, input_path, listing_path)
  else
    list_command = build_posix_list_command(archive_tool_path, input_path, listing_path)
  end
  ...
  local renamed_files = ZipArchive.allocate_audio_paths(audio_files, output_dir)
  local extract_result = ExtractWriter.run(archive_tool_path, input_path, audio_files, extract_log_path, timeout_ms)
  ...
  -- Existing files or partial output cannot substitute for a successful batch.
  if not extract_log_text:match("archive_extract_ok=true%s*$") then
    return nil, build_stage_failure(archive_extract_stage_label(), archive_tool_path, input_path, extract_log_path, extract_result,
      "Extraction did not complete. No stems were imported; existing files were not overwritten.")
  end

  for _, item in ipairs(audio_files) do
    local local_path = item.path
    local size = Files.file_size(local_path)
    if (size or 0) <= 0 or (item.expected_size and size ~= item.expected_size) then
      keep_or_cleanup_logs_on_failure(true)
      return nil, build_stage_failure(
        ...
        extract_log_path,
        extract_result,
        "Extracted audio file is missing, empty, or has the wrong size: " .. tostring(local_path)
      )
    end
